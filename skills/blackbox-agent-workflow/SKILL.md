...
- Representative Run priority within the candidate scope is manual baseline, then the completed Run with the highest valid primary metric, then the latest Run. A manual baseline is a Run explicitly bound to the baseline node of an active research map; Branch/Research bindings are dynamic evidence, not pinned Runs. General selection uses `strategy.summary.sharpe`; map selection uses the map's `primary_metric`. This selection does not imply research approval or bypass result quality gates.
