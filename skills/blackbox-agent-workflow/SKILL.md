...
- For CLI performance results, prefer `bbox run publish-performance`. It safely infers standard date/value columns, copies the chosen value into `series_values`, adds typed performance metadata, converts explicitly declared decimal summary percentages to percentage points, uploads with stable derived idempotency keys, and validates the stored run. Pass `--finish` to finish only after validation succeeds.
- Always declare `--mode` and `--summary-unit` on `publish-performance`; unit semantics are never guessed. Use `--dry-run` to inspect normalization without writing and `--agent-output` for compact diagnostics.
