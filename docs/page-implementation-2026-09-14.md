...
| 页面 | 实际变化 | 指南依据 |
|---|---|---|
| 总览 | 增加页面标题和全局观察范围；按用户最新偏好，标题后依次展示活动历史、项目，再展示主要统计及结果/复核模块，其余模块相对顺序保持 | §2.1、§3.1–3.3 |
...
后续可读性调整：全局表头由 11px/500/暗灰改为 13px/600/主题次级文字色，并增加轻微底色；地图比较表头、分区标题、提示说明和侧栏计数提升至 12px，排序箭头取消半透明。项目研究线页面已验证深浅色实际效果，深色表头计算样式为 `13px / 600 / rgb(184,179,170)`，背景为 `rgb(33,32,29)`。截图为本目录下 `page-implementation-2026-09-14/readability-dark.png` 与 `readability-light.png`。此次构建与 `git diff --check` 通过。
